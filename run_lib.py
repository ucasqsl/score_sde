...
def train(config, workdir):
    ...
    for step in range(
        ...
        batch = jax.tree_util.tree_map(
            lambda x: scaler(x._numpy()), next(train_iter)
        ...
        if step % config.training.eval_freq == 0:
            eval_batch = jax.tree_util.tree_map(
                lambda x: scaler(x._numpy()), next(eval_iter)
            )  # pylint: disable=protected-access
            rng, *next_rng = jax.random.split(rng, num=jax.local_device_count() + 1)
            next_rng = jnp.asarray(next_rng)
...
def evaluate(config, workdir, eval_folder="eval"):
    ...
    if config.eval.enable_loss:
        ...
        p_eval_step = jax.pmap(
            functools.partial(jax.lax.scan, eval_step), axis_name="batch"
        )

    # Create data loaders for likelihood evaluation. Only evaluate on uniformly dequantized data
    train_ds_bpd, eval_ds_bpd, _ = datasets.get_dataset(
        config, additional_dim=None, uniform_dequantization=True, evaluation=True
    )
    ...
    eval_meta = EvalMeta(
        ckpt_id=config.eval.begin_ckpt, sampling_round_id=-1, bpd_round_id=-1, rng=rng
    )
    ...
    for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
            ...
            for repeat in range(begin_repeat_id, bpd_num_repeats):
                ...
                for batch_id in range(begin_batch_id, len(ds_bpd)):
                    ...
                    eval_meta = eval_meta.replace(
                        ckpt_id=ckpt, bpd_round_id=bpd_round_id, rng=rng
                    )
        ...
        if config.eval.enable_sampling:
            ...
            for r in range(begin_sampling_round, num_sampling_rounds):
                ...
                eval_meta = eval_meta.replace(
                    ckpt_id=ckpt, sampling_round_id=r, rng=rng
                )
            ...
            eval_meta = eval_meta.replace(
                ckpt_id=ckpt, sampling_round_id=num_sampling_rounds - 1, rng=rng
            )
            checkpoints.save_checkpoint(
                eval_dir,
                eval_meta,
